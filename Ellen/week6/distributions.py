...
def mean(vals): ## latest version of a function to calculate the mean of a string of numbers
    addup=0.0
    for i in vals:
        addup += i
    return addup / float(len(vals))

def std(vals): ## function to calculate standard deviation
    m = mean(vals)
    diffsum = 0.0
    for i in vals:
        diffsum += (i-m) ** 2.0
...
class uniform: #Set uniform class, this one looks like rectangle with low and high x threshold
...
class normal: #Defining normal class distribution
...
class binomial: #Defing binomial distribution class
...
class exponential: #Defining expoential distribution class
